...
def process_x(x, n_clusters, drop_scale):
    """
    ...
    return pd.DataFrame(
        FeatureAgglomeration(n_clusters=n_clf).fit_transform(x),
        index=gene_names)
...
def return_x_y(df, causal_genes, k_clusters, drop_scale):
    """
    ...
    x = process_x(prepare_df(df), k_clusters, drop_scale)
    return x, y
