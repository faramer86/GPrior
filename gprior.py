...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--n_bootstrap', type=int, required=False, metavar='',
                        help='(default=30) Number of bootstraps for PU Bagging')
    ...
    parser.add_argument('--drop_aes', action='store_true', required=False,
                        ...
                        Instead of optimal combination - individual predictions and their mean will be returned')
    parser.add_argument('--drop_scale', action='store_true', required=False,
                        help='(positional; default=False) Whether to scale features using interquartile range (IQR).')
    ...
    if args.drop_scale:
        print('NOTE: You do not not scale features!')

    if args.add_features:
        input_file = FEtoolbox.add_features(input_file, causal_genes)
        input_file.to_csv(args.output, sep='\t', index=False)
        sys.exit("Features are successfully added! \nNow you can use returned file as an input!")
